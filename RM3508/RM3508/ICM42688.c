...
void ICM42688_Init()
{
    ...
    wakeOnMotionInit();
    setWOMTh(ALL,39);
    setODRAndFSR(GYRO, ODR_200HZ, FSR_2);
}
...
float Sample_Array1[2000] = {0.0f};
float Sample_Array2[2000] = {0.0f};
void Data_Init(Kal_Filter* K_Filt,get_yaw *G_yaw)
{
...
//		gyro->x = 0;
//	}
//		if(gyro->y<=gyro->Dead_field )
//	{
//		gyro->y = 0;
//	}
//		if(gyro->z<=gyro->Dead_field )
//	{
//		gyro->z = 0;
//	}
//	gyro->x = getGyroDataX();
...
//	gyro->z = getGyroDataZ();
//	
//	accel->x =getAccelDataX();
//	accel->y =getAccelDataY();
//	accel->z =getAccelDataZ();
}
 ...
 * @retval None
  */
void ICM42688_yaw_adjust(Kal_Filter* K_Filt,get_yaw *G_yaw)
{
	G_yaw->OmigaZ  = KalmanFilter (K_filt,G_yaw->OmigaZ)-G_yaw->OmigaOffset;  //
	
	
	if (fabs(G_yaw->OmigaZ) < ICM_ANGLE_DEATH) // 把角速度为0时的值存入采样数组
	{
		G_yaw->Sample_Array[G_yaw->SampleIndex] = G_yaw->OmigaZ; // 滑动平均求零点偏移
		G_yaw->SampleIndex++;
		if (G_yaw->SampleIndex == SAMPLE_NUM)
			G_yaw->SampleIndex = 0;

		// 计算动态补偿
		arm_mean_f32(G_yaw->Sample_Array, SAMPLE_NUM, &G_yaw->OmigaOffsetTemp); // 滑动平均滤波，长度为10
	}
	else
		G_yaw->OmigaZ -= G_yaw->OmigaOffsetTemp;

	if (fabs(G_yaw->OmigaZ) < ICM_ANGLE_DEATH) // 死区
		G_yaw->OmigaZ = 0.0f;
 ...
 * @retval None
  */
//float ICM42688_VX_adjust(Kal_Filter* K_Filt,get_yaw *G_yaw)
//{
//	G_yaw->AccelX  = KalmanFilter (K_filt,G_yaw->AccelX)-G_yaw->AccelXOffset;  //
//	
//	
//	if (fabs(G_yaw->AccelX) < ICM_ACCELX_DEATH) // 把角速度为0时的值存入采样数组
//	{
//		G_yaw->Sample_Array1[G_yaw->SampleIndex1] = G_yaw->AccelX; // 滑动平均求零点偏移
//		G_yaw->SampleIndex1++;
//		if (G_yaw->SampleIndex1 == SAMPLE_NUM)
//			G_yaw->SampleIndex1 = 0;

//		// 计算动态补偿
//		arm_mean_f32(G_yaw->Sample_Array1, SAMPLE_NUM, &G_yaw->AccelXOffsetTemp); // 滑动平均滤波，长度为10
//	}
//	else
//		G_yaw->AccelX -= G_yaw->AccelXOffsetTemp;

//	if (fabs(G_yaw->AccelX) < ICM_ACCELX_DEATH) // 死区
//		G_yaw->AccelX = 0.0f;

//	/*更新角度*/
//	if (G_yaw->AccelX + G_yaw->LastAccelX > 0.0f){
//	  G_yaw->Velocity_X += (G_yaw->AccelX + G_yaw->LastAccelX) * 0.0001f * POSI_SCALE;
//	}
//	else{
//	  G_yaw->Velocity_X += (G_yaw->AccelX + G_yaw->LastAccelX) * 0.0001f * NEGA_SCALE;
//	}
//	
//	

//	G_yaw->LastAccelX = G_yaw->AccelX;
//	
//	
//	return G_yaw->Velocity_X;
//}
 ...
 * @retval None
  */
//void ICM42688_X_adjust(Kal_Filter* K_Filt,get_yaw *G_yaw)
//{
//	G_yaw->Velocity_X  = KalmanFilter (K_filt,G_yaw->Velocity_X)-G_yaw->AccelXOffset;  //
////	
////	
////	if (fabs(G_yaw->Velocity_X) < ICM_VX_DEATH) // 把角速度为0时的值存入采样数组
////	{
////		G_yaw->Sample_Array3[G_yaw->SampleIndex3] = G_yaw->Velocity_X; // 滑动平均求零点偏移
////		G_yaw->SampleIndex3++;
////		if (G_yaw->SampleIndex3 == SAMPLE_NUM)
////			G_yaw->SampleIndex3 = 0;

////		// 计算动态补偿
////		arm_mean_f32(G_yaw->Sample_Array3, SAMPLE_NUM, &G_yaw->VXOffsetTemp); // 滑动平均滤波，长度为10
////	}
////	else
////		G_yaw->Velocity_X -= G_yaw->VXOffsetTemp;

//	if (fabs(G_yaw->Velocity_X) < ICM_VX_DEATH) // 死区
//		G_yaw->Velocity_X = 0.0f;

//	/*更新角度*/
//	if (G_yaw->Velocity_X + G_yaw->LastVelocity_X > 0.0f){
//	  G_yaw->Distance_X += (G_yaw->Velocity_X + G_yaw->LastVelocity_X) * 0.0001f * POSI_SCALE;
//	}
//	else{
//	  G_yaw->Distance_X += (G_yaw->Velocity_X + G_yaw->LastVelocity_X) * 0.0001f * NEGA_SCALE;
//	}
//	G_yaw->LastVelocity_X = G_yaw->Velocity_X;
//	
//}
