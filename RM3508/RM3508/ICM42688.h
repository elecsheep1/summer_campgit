...
typedef struct get_yaw
{
	
	float Yaw ;				   //校准后航向角
	
	float Velocity_X;    //x轴路程
	float Velocity_Y;    //x轴路程
	
	float LastVelocity_X;
	float LastVelocity_Y;

	
	float Distance_X;    //x轴路程
	float Distance_Y;    //x轴路程
  ...
  uint8_t SampleIndex;
	...
	float Sample_Array1[SAMPLE_NUM];
  uint8_t SampleIndex1;
	
	float Sample_Array2[SAMPLE_NUM];
  uint8_t SampleIndex2;
	
	float Sample_Array3[SAMPLE_NUM];
  uint8_t SampleIndex3;
	
	
	float OmigaOffset;  		//上一次记录角速度偏移值    
	float OmigaOffsetTemp;
	
  float AccelXOffset;  		//上一次记录x加速度偏移值    
	float AccelXOffsetTemp;
	
	float AccelYOffset;  		//上一次记录y加速度偏移值    
	float AccelYOffsetTemp;
...
extern Accel  accel;
extern Accel *Accel_a;

extern get_yaw Get_yaw;
extern get_yaw *G_yaw;

extern float ICM_ANGLE_DEATH;
