...
//	  static uint8_t UI = 0;
//	  static uint32_t Counter_RGB = 0;
//	  for(int i=0,j=13;i<=12&&j<=17;i++,j++)
//		States_RGB_Light[i] = States_RGB_Light[j] = 0.5f;
//	  for(int i=5;i<=12;i++)
//		States_RGB_Light[i] -= 0.01f;
//	  for(int i=5;i<=12;i++)
//		More_RGB[i] = States_RGB[i];
//	  for(int i=0;i<=4;i++)
//	    More_RGB[i] = Black_Color;
//	  for(int i=13;i<=17;i++) 
//	    More_RGB[i] = Black_Color; 
//	  Counter_RGB = (Counter_RGB + 1)%500;
//	  UI = Counter_RGB / 100;
// 	  More_RGB[UI] = More_RGB[UI + 13] = Blue_Color;
//	  RGB_Color_Convert_Pro_with_Light(&htim4, TIM_CHANNEL_3, States_RGB_Light);
//}
