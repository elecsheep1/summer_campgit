...
//	//速度&角度计算分母
//	double count_denominator = sqrt( pow(motion_lf,2) + pow(motion_rf,2) );
	//调整位置环速度限幅
...
{
	//将角度换算为线圈数
	double r = rotation * R_WHEEL_TO_CENTER * DIS_TO_POS;
//	//归零角度计数
//	zero_lf_pos();zero_lb_pos();zero_rf_pos();zero_rb_pos();
	//计算轮子需旋转角度
	double count_lf = r;
