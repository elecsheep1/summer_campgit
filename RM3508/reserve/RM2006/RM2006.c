...
M2006_PID M2006_Pos_Pid[8] = //云台0.2, 0.009 0.01  底盘0.2 0 0
{
...
//用于存储电机反馈的全局变量
uint8_t RM2006_Feedback_Buf[8][7];		//电机反馈值(全局变量)
int RM2006_Pos[8];					//每一个元素对应一个ID的电机的信息
uint8_t RM2006_Sendbuf1[8] = {0};  //CAN发送数据
uint8_t RM2006_Sendbuf2[8] = {0};  //CAN发送数据



/*********************************************************************************
  *@  name      : RM2006_Set_I
  *@  function  : RM2006电机电流设置
  *@  input     : 目标电流，电机id
  *@  output    : 成功返回0，失败返回1
*********************************************************************************/
uint8_t RM2006_Set_I(int target_i,uint8_t motor_id)
{
	if( motor_id>=1 && motor_id<=8 ) 
	{
        ...
        if(motor_id <=4 )   //前四个ID对应的sendID标识符是0x200
        {
            ...
            RM2006_Sendbuf2[2*motor_id-2]=target_i>>8;                  //电流值高8位
            RM2006_Sendbuf2[2*motor_id-1]=target_i & 0x00ff;            //电流值低8位
        ...
        RM2006_Sendbuf1[2*motor_id-2]=target_i>>8;                  //电流值高8位
        RM2006_Sendbuf1[2*motor_id-1]=target_i & 0x00ff;            //电流值低8位
...
void RM2006_Set_Pos(float pos,int ID)
{
	...
	float goal_cnt;
	goal_cnt=pos;
//	goal_cnt=RM2006_Ang2Cnt(angle,ID);//该函数为电机角度转cnt
...
/*********************************************************************************
  ...
  *@  function  : 获取RM2006电机的反馈并存入全局变量RM2006_Feedback_Buf[8][7];
...
/*********************************************************************************
  *@  name      : RM2006_Pos_Rec
  *@  function  : 获取RM2006电机的反馈的位置信息  //累积路程
  *@  input     : 电机id号
  *@  output    : 对应id电机的位置信息,读取失败返回-1
*********************************************************************************/
static int32_t	RM2006_base[8] = {0};	//用来标记已经转过的编码器线数，一圈8192
...
void RM2006_Pos_Rec(uint8_t motor_id)
{
	...
	if ( RM2006_tmp[id] - RM2006tmp_pre[id] > 4095 )  //转过8191到0时记录圈数
		RM2006_base[id] -= 8191;
